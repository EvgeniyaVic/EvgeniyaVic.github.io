    ...
    def setUp(self) -> None:
        ...
        lst = [
            ...
            Train(name='tr8', from_city=self.city_E, to_city=self.city_D,
                  travel_time=5),
            Train(name='tr1', from_city=self.city_D, to_city=self.city_E,
                  travel_time=4),
        ]
        Train.objects.bulk_create(lst)

    def test_model_city_duplicate(self):
        #Тестирование возниклновения ошибки при создании дубля города
        city = City(name="A")
        with self.assertRaises(ValidationError):
            city.full_clean()

    def test_model_train_duplicate(self):
        train = Train(name='tr1', from_city=self.city_A, to_city=self.city_B,
                      travel_time=129)
        with self.assertRaises(ValidationError):
            train.full_clean()

    def test_model_train_train_duplicate(self):
        train = Train(name='tr1567', from_city=self.city_A, to_city=self.city_B,
                      travel_time=9)
        with self.assertRaises(ValidationError):
            train.full_clean()
        try:
